...
-- 3. Participantes de Grupos (Relación M:N)[cite: 1]
CREATE TABLE group_participants (
    group_id UUID NOT NULL,
    user_id UUID NOT NULL,
    accumulated_points INT NOT NULL DEFAULT 0,[cite: 1]
...
CREATE TABLE matches (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    external_api_id VARCHAR(50) UNIQUE,             -- ID de sincronización con thesportsdb.com[cite: 1]
    home_team VARCHAR(100) NOT NULL,
    away_team VARCHAR(100) NOT NULL,
    date_time TIMESTAMP WITH TIME ZONE NOT NULL,[cite: 1]
    phase VARCHAR(50) NOT NULL,[cite: 1]
    status match_status NOT NULL DEFAULT 'PENDING',[cite: 1]
    home_score INT DEFAULT NULL,[cite: 1]
    away_score INT DEFAULT NULL,[cite: 1]
    stadium VARCHAR(150) NOT NULL,
    city VARCHAR(100) NOT NULL,[cite: 1]
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- 5. Pronósticos (Predictions)[cite: 1]
CREATE TABLE predictions (
    ...
    points_earned INT DEFAULT NULL,[cite: 1]
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- Optimización de Login (Búsqueda insensible a mayúsculas/minúsculas)[cite: 1]
...
-- Acceso rápido para unirse a grupos vía código[cite: 1]
CREATE INDEX idx_groups_invite_code ON groups (invite_code);

-- Tabla de posiciones en tiempo real orden descendente[cite: 1]
CREATE INDEX idx_group_participants_leaderboard ON group_participants (group_id, accumulated_points DESC);

-- Consultas de calendario y validación de bloqueos de tiempo en API[cite: 1]
CREATE INDEX idx_matches_date_status ON matches (date_time, status);

-- Índices de llaves foráneas para acelerar los Joins del Dashboard[cite: 1]
...
--------------------------------------------------------------------------------
-- REGLA 1: TIME-LOCK TRIGGER (Protección de datos ante fraude de apuestas)[cite: 1]
--------------------------------------------------------------------------------
...
BEGIN
    ...
    -- Si la hora actual es mayor o igual a la hora del partido, se bloquea la transacción[cite: 1]
...
--------------------------------------------------------------------------------
-- REGLA 2: PROCESAMIENTO AUTOMÁTICO DE PUNTOS[cite: 1]
--------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_process_match_results_and_points()
RETURNS TRIGGER AS $$
BEGIN
    -- Se activa únicamente si el estado cambia estrictamente a 'FINISHED'[cite: 1]
    IF NEW.status = 'FINISHED' AND (OLD.status IS DISTINCT FROM 'FINISHED') THEN
        
        -- A) Evaluación de pronósticos y asignación de puntuación[cite: 1]
        UPDATE predictions
        SET points_earned = CASE
            -- Exacto: Gana 3 puntos[cite: 1]
            WHEN predicted_home = NEW.home_score AND predicted_away = NEW.away_score THEN 3
            -- Acierto de Ganador Local: 1 punto[cite: 1]
            WHEN (NEW.home_score > NEW.away_score AND predicted_home > predicted_away) THEN 1
            -- Acierto de Ganador Visitante: 1 punto[cite: 1]
            WHEN (NEW.home_score < NEW.away_score AND predicted_home < predicted_away) THEN 1
            -- Acierto de Empate: 1 punto[cite: 1]
            WHEN (NEW.home_score = NEW.away_score AND predicted_home = predicted_away) THEN 1
        ...
        -- B) Recálculo masivo y actualización de la tabla intermedia de posiciones[cite: 1]
